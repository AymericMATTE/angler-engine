...
#include "ecs/components/collision-tag-component.h"
#include "ecs/components/state-machine-component.h"
...
#include "ecs/scripts/default-cam-script.h"
#include "ecs/systems/default-cam-system.h"
...
namespace angler {
    ...
    Application& Application::get() {
        if (m_inst == nullptr) {
            // TEMP : Load the config from a file
            m_settings = loadSettingsFromFile(os::getFileInDirectory(RES_DIR, "settings.angler"));
    ...
    int Application::run() {
        ...
        while (m_window.isOpen()) {
            ...
            if (m_ecs.cleanup()) {
                OnApplicationReset.invoke();
                OnApplicationReset.clearListeners();
